...
final class DefaultReminderService: ReminderService {
    ...
    func fetchReminders(completion: @escaping ([Reminder]) -> Void) {
        ...
        for _ in 0 ..< 3 {
            group.enter()
            dataSource.fetchReminders { fetched in
                lock.lock()
    ...
    func fetchRemindersAsync() async -> [Reminder] {
        ...
        async let first = dataSource.fetchReminders()
        async let second = dataSource.fetchReminders()
        async let third = dataSource.fetchReminders()
